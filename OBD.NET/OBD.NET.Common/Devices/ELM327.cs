...
namespace OBD.NET.Common.Devices
{
    public class ELM327 : SerialDevice
    {
        ...
        protected byte GetModeByte() => (byte)Mode;
        ...
        public virtual void RequestData<T>(int? maxResponses = default)
        ...
        /// <param name="maxResponses">The maximum number of responses to wait for from the control module</param>
        public virtual void RequestData(int pid, byte? modeOverride = default, int? maxResponses = default)
        {
            Logger?.WriteLine("Requesting PID " + pid.ToString("X2") + " ...", OBDLogLevel.Debug);

            string command = (modeOverride ?? GetModeByte()).ToString("X2") + pid.ToString("X2");
        ...
        /// <typeparam name="T"></typeparam>
        /// <returns></returns>
        public virtual async Task<T> RequestDataAsync<T>(int? maxResponses = default)
            where T : class, IOBDData, new()
        {
            Logger?.WriteLine("Requesting Type " + typeof(T).Name + " ...", OBDLogLevel.Debug);
            int   pid  = ResolvePid<T>();
        ...
        /// </summary>
        /// <returns></returns>
        public virtual async Task<IOBDData> RequestDataAsync(Type type, byte? modeOverride = default, int? maxResponses = default)
        {
            Logger?.WriteLine("Requesting Type " + type.Name + " ...", OBDLogLevel.Debug);
            int pid = ResolvePid(type);
            return await RequestDataAsync(pid, modeOverride, maxResponses) as IOBDData;
        }
        ...
        /// <param name="modeOverride">Optional mode override when sending the command</param>
        /// <param name="maxResponses">The maximum number of responses to wait for from the control module</param>
        public virtual async Task<object> RequestDataAsync(int pid, byte? modeOverride = null, int? maxResponses = default)
        {
            Logger?.WriteLine("Requesting PID " + pid.ToString("X2") + " ...", OBDLogLevel.Debug);
        ...
        protected override object ProcessMessage(string message)
        {
            ...
            {
                CanError?.Invoke(this, new CanErrorEventArgs { Message = message });
            }
